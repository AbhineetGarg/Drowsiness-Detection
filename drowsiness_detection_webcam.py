    ...
    if result:
        ...
        if outputs.multi_face_landmarks:
            ...
            if MAR_ratio < 1.8:
                ...
                cv2.putText(image, "Drowsy Alert", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, COLOR_RED, 2)
                print(
                    f"Drowsy Alert: It Seems you are sleeping.. please wake up {i} , ratio : {MAR_ratio:.4f} , eyes : {ratio:.4f}")

        ## Draw the landmarks from mouth on the image
        original_image = image
        new_image = cv2.circle(image, (
        int(outputs.multi_face_landmarks[0].landmark[mouth_coordinates["mouth_upper"]].x * image.shape[1]),
        int(outputs.multi_face_landmarks[0].landmark[mouth_coordinates["mouth_upper"]].y * image.shape[0])), 1,
                               COLOR_BLUE, -1)
        new_image = cv2.circle(new_image, (
        int(outputs.multi_face_landmarks[0].landmark[mouth_coordinates["mouth_lower"]].x * image.shape[1]),
        int(outputs.multi_face_landmarks[0].landmark[mouth_coordinates["mouth_lower"]].y * image.shape[0])), 1,
                               COLOR_BLUE, -1)
        new_image = cv2.circle(new_image, (
        int(outputs.multi_face_landmarks[0].landmark[mouth_coordinates["mouth_left"]].x * image.shape[1]),
        int(outputs.multi_face_landmarks[0].landmark[mouth_coordinates["mouth_left"]].y * image.shape[0])), 1,
                               COLOR_BLUE, -1)
        new_image = cv2.circle(new_image, (
        int(outputs.multi_face_landmarks[0].landmark[mouth_coordinates["mouth_right"]].x * image.shape[1]),
        ...
        new_image = cv2.circle(new_image, (
        int(outputs.multi_face_landmarks[0].landmark[eye_coordinates["left_eye_top"]].x * image.shape[1]),
        int(outputs.multi_face_landmarks[0].landmark[eye_coordinates["left_eye_top"]].y * image.shape[0])), 1,
                               COLOR_BLUE, -1)
        new_image = cv2.circle(new_image, (
        int(outputs.multi_face_landmarks[0].landmark[eye_coordinates["left_eye_bottom"]].x * image.shape[1]),
        int(outputs.multi_face_landmarks[0].landmark[eye_coordinates["left_eye_bottom"]].y * image.shape[0])), 1,
                               COLOR_BLUE, -1)
        new_image = cv2.circle(new_image, (
        int(outputs.multi_face_landmarks[0].landmark[eye_coordinates["left_eye_left"]].x * image.shape[1]),
        int(outputs.multi_face_landmarks[0].landmark[eye_coordinates["left_eye_left"]].y * image.shape[0])), 1,
                               COLOR_BLUE, -1)
        new_image = cv2.circle(new_image, (
        int(outputs.multi_face_landmarks[0].landmark[eye_coordinates["left_eye_right"]].x * image.shape[1]),
        ...
        new_image = cv2.circle(new_image, (
        int(outputs.multi_face_landmarks[0].landmark[eye_coordinates["right_eye_top"]].x * image.shape[1]),
        int(outputs.multi_face_landmarks[0].landmark[eye_coordinates["right_eye_top"]].y * image.shape[0])), 1,
                               COLOR_BLUE, -1)
        new_image = cv2.circle(new_image, (
        int(outputs.multi_face_landmarks[0].landmark[eye_coordinates["right_eye_bottom"]].x * image.shape[1]),
        int(outputs.multi_face_landmarks[0].landmark[eye_coordinates["right_eye_bottom"]].y * image.shape[0])), 1,
                               COLOR_BLUE, -1)
        new_image = cv2.circle(new_image, (
        int(outputs.multi_face_landmarks[0].landmark[eye_coordinates["right_eye_left"]].x * image.shape[1]),
        int(outputs.multi_face_landmarks[0].landmark[eye_coordinates["right_eye_left"]].y * image.shape[0])), 1,
                               COLOR_BLUE, -1)
        new_image = cv2.circle(new_image, (
        int(outputs.multi_face_landmarks[0].landmark[eye_coordinates["right_eye_right"]].x * image.shape[1]),
        ...
        cv2.putText(new_image, f"MAR: {MAR_ratio:.2f}", (image.shape[1] - 200, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    COLOR_RED, 2)
        cv2.putText(new_image, f"EAR: {ratio:.2f}", (image.shape[1] - 200, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, COLOR_RED,
                    2)

        cv2.imshow("Drowsiness Detection", original_image)
